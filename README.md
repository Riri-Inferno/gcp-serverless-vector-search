...
| 埋め込みモデル | **`gemini-embedding-2`** (3072次元) via **Google AI Studio API** | テキスト・画像・動画・音声・PDF をすべて同じベクトル空間に射影できる。次元は柔軟（128〜3072）、最大精度の 3072 を採用（ADR 0015） |
...
```
[Client]
   │ HTTPS (vector-search.riri-inferno.com)
   ▼
...
[API Gateway (GCP)]             認証 / レート制限 / OpenAPI ルーティング
   │
   ▼
[Cloud Functions (2nd gen)] ── Google AI Studio (gemini-embedding-2, 3072d)
