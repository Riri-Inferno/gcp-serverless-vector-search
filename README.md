...
- 同一ベクトル空間で **モダリティ別の高精度な近傍検索**（画像 → 画像、テキスト → テキスト、手書き → 画像 が強力）
- **擬似クロスモーダル検索** (`mix_modalities` トグル ON で同モダリティ + 異モダリティを半々で取得・merge、modality gap への対処 / 詳細は [ADR 0019](docs/adr/0019-mix-modalities-filter-based-cross-modal.md))
- テキスト・画像・手書きキャンバス・クリップボード貼り付け（Ctrl+V）で検索
- 検索結果を **すべて / 画像 / テキスト** でフィルタリング
- ドキュメントの登録・削除（GCS メディアファイルも連動削除）

Gemini Embedding API（`gemini-embedding-2`）でテキスト・画像・動画・音声・PDF を同一ベクトル空間（2048 次元）に射影し、Firestore のネイティブベクトル検索（`find_nearest`）で近傍探索します。joint embedding と謳う公式仕様にもかかわらず実用上の modality gap が確認されており（[2026-06-23 計測](docs/measurements/2026-06-23-modality-gap.md)）、`mix_modalities` 機能でその迂回を提供しています。
...
本プロジェクトに関するお問い合わせや、採用・案件のご提案につきましては、下記プラットフォームまたは経由エージェント様を通じてご連絡いただけますと幸いです。

- **GitHub Issues / Discussions**: バグ報告や機能要望はリポジトリの [Issues](https://github.com/Riri-Inferno/gcp-serverless-vector-search/issues) へ直接起票してください。
- **職務経歴書 / エージェント窓口**: 採用に関わるご連絡は、提示しているポートフォリオ媒体、または仲介エージェント様のリファラル窓口宛てにお願いいたします。
