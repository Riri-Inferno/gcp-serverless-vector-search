...
設計時には `roles/owner` を初期 bootstrap の候補として検討したが、`home-raspi-iac` の実績パターン (`roles/editor` + 個別 admin role の組み合わせ) を踏襲して **owner を経由せず最初から後者の構成で開始** した。これにより owner 付与/剥奪のフェーズをスキップしつつ、必要な resource-level IAM admin を網羅できる。

実際に付与しているロール（`terraform/bootstrap/ci_sa.tf` 参照）:

| ロール                                  | 用途                                              |
| --------------------------------------- | ------------------------------------------------- |
| `roles/editor`                          | 広範な resource CRUD（IAM 系は含まない）          |
| `roles/iam.serviceAccountAdmin`         | SA CRUD                                           |
| `roles/iam.workloadIdentityPoolAdmin`   | WIF 管理                                          |
...
| `roles/run.admin`                       | Cloud Functions Gen2（内部 Cloud Run）            |
...
| `roles/cloudkms.admin`                  | KMS key（SOPS 用、ADR 0006）                      |

WIF Provider の repository pin と CI SA impersonation binding を境界として扱い、信頼境界はこの 2 段で担保する。運用が安定したら、実際に必要だった権限を見ながら custom role へ縮小する余地は残す。
